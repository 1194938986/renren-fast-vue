  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('product:brand:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除
        </el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="showStatus" header-align="center" align="center" label="显示状态[0-不显示；1-显示]">
        <template slot-scope="scope">
          //  inactive-value  active-value对应开关的值，v-model再绑定一个值。数据库-->showStatus-->v-model     v-model-->@change-->数据库
          <el-switch v-model="scope.row.showStatus" active-color="#13ce66" inactive-color="#ff4949" :active-value="1" :inactive-value="0"
            @change="updateBrandStatus(scope.row)">
            //   slot-scope是上一级的插槽，可以访问到上一级的（也就是直属上级）的data
          </el-switch>
        </template>
      </el-table-column>
